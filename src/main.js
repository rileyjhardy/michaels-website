...
const Main = () => {
    ...
    useEffect(() => {
        ...
        if (renderState.displayContact){
            disableBodyScroll(document.querySelector('#contactid'));
        } else if (renderState.displayGear){
            disableBodyScroll(document.querySelector('#gearid'));
        } else if (renderState.displayNav){
            disableBodyScroll(document.querySelector('#navid'));
        } else if (renderState.playFullReel){
            disableBodyScroll(document.querySelector('#fullreel'));
        } else if (playerData.currentData !== null){
    ...
    return (
        <Router>                
            <div className='main'>            
                ...
                <IsolatedPlayer renderstate = {renderState} data = {playerData} setplayerdata = {setPlayerData} />
                ...
                <Contact renderstate = {renderState} setrenderstate = {setRenderState} />         
            </div>                
        </Router>
        )
    };
